...
const Toast = ({type, title, description, icon}) => {
    const Container = tw(colorVariants[type])`fixed flex flex-row rounded-[12px] top-[10vh] -translate-x-1/2 inset-x-1/2 dt:h-24 w-max border-l-[12px] p-4 gap-x-2 dt:gap-x-4`;
    ...
    const XButton = tw.button(colorVariants[type])`cursor-pointer dt:text-xl`;
